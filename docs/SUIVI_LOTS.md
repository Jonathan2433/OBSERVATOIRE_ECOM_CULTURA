...
| D36 | 2026-09-15 | **Dépôt multiple** : un lot accepte N exports (`fichiers[]`, plafond 20), la source de chacun étant identifiée à la lecture | Un mois se dépose en une fois. C'est la reconnaissance au jeu de colonnes (D33) qui le rend simple : sans elle il aurait fallu un champ de formulaire par source. Un fichier non reconnu **fait échouer le lot en le nommant** — traiter un mois amputé d'une source fausserait les volumes. Les champs `mdtc`/`mopinion` restent acceptés et les lots en base se relisent |
...
| D52 | 2026-09-21 | **Carte « Modèle actif » retirée** des Tableaux de bord (global et par lot) ; **toutes les évolutions basculent en % relatif** (`(actuel − référence) / référence`) au lieu de points — thèmes, taux de revue, bi-thèmes, signaux **et** satisfaction, ce qui **révise D-41**. Une référence à 0 est distinguée explicitement (« nouveau ») plutôt que d'afficher une division par zéro. Le endpoint `GET /api/kpi/model` reste disponible (livrable API inchangé), seul son usage en dashboard disparaît | Demande métier (PO) : la lecture en points était jugée moins parlante que l'évolution relative, y compris pour la satisfaction où D-41 avait pourtant choisi l'inverse le 17/09 pour la même raison inversée (`+0,1 pt` mal lu comme `+10 %`). Arbitrage explicite du PO pour uniformiser tous les indicateurs d'évolution sur le même format |
| D53 | 2026-09-22 | **Refonte du bandeau « Périmètre d'analyse »** (Tableaux de bord global et par lot, Résultats, Revue) : les filtres Sources et Période sont regroupés visuellement sous deux libellés distincts au lieu d'un unique bloc mélangé, le compteur de filtres actifs et le bouton Réinitialiser restent toujours visibles dans l'en-tête (le bouton se désactive au lieu de disparaître). Aucun filtre ajouté ni retiré, aucun changement d'API | Demande métier : la présentation existante mélangeait pastilles de sources et champs de date sur une seule ligne, sans regroupement ni alignement, avec un texte d'aide sur les dates déconnecté du champ concerné. Composant partagé (`AnalysisFiltersBar`) : correction faite une seule fois pour les 4 écrans qui l'utilisent |
