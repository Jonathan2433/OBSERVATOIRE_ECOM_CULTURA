#!/usr/bin/env python3
"""Recette V4 — moteur Ollama (LLM local).

Vérifie le moteur Ollama de bout en bout SANS Ollama réel (HTTP mocké) et SANS
...
- O3 : détection Ollama (`/api/tags`), `available` dynamique, synchro registre.
...
from worker import ollama_predictor as op  # noqa: E402
...
CFG = {
    ...
    "ollama": {"fallback_theme": "Autre / Non classé", "guardrails": {
...
def run() -> None:
    ...
    # --- 2. Prompt -----------------------------------------------------------
    p = op.build_ollama_prompt(TAXO, "le colis est arrivé cassé", 2)
    ...
    r = op.map_ollama_response(raw_ok, "colis cassé", 2, TAXO, CFG, SENT_LABELS)
    ...
    rb = op.map_ollama_response(raw_bad, "texte", None, TAXO, CFG, SENT_LABELS)
    ...
    rn = op.map_ollama_response(raw_niv1, "texte", None, TAXO, CFG, SENT_LABELS)
    ...
    raw_sent = {"themes": [{"niv1": N1, "niv2": N2, "sentiment": "Enthousiaste", "confidence": 0.9}],
                "signaux": {}}
    rs = op.map_ollama_response(raw_sent, "texte", None, TAXO, CFG, SENT_LABELS)
    ...
    re = op.map_ollama_response({}, "texte", None, TAXO, CFG, SENT_LABELS)
    ...
    rv = op.map_ollama_response(raw_ok, "   ", None, TAXO, CFG, SENT_LABELS)
    ...
    rr = op.map_ollama_response(raw_rup, "je pars chez la concurrence", 5, TAXO, CFG, SENT_LABELS)
    ...
    rc = op.map_ollama_response(raw_case, "texte", None, TAXO, CFG, SENT_LABELS)
    ...
    raw_sc = {"themes": [{"niv1": N1, "niv2": N2, "sentiment": "NEGATIF", "confidence": 0.9}],
              "signaux": {}}
    rsc = op.map_ollama_response(raw_sc, "texte", None, TAXO, CFG, SENT_LABELS)
    ...
    rj = op.map_ollama_response({}, "texte", None, TAXO, CFG, SENT_LABELS)
    ...
    raw_dup = {"themes": [{"niv1": N1, "niv2": N2, "sentiment": "Négatif", "confidence": 0.8},
               ...
               "signaux": {}}
    rd = op.map_ollama_response(raw_dup, "texte", None, TAXO, CFG, SENT_LABELS)
    ...
    raw_conf = {"themes": [{"niv1": N1, "niv2": N2, "sentiment": "Négatif", "confidence": 0.95},
                ...
                "signaux": {}}
    rcf = op.map_ollama_response(raw_conf, "texte", None, TAXO, CFG, SENT_LABELS)
    ...
    pv = op.build_ollama_prompt(TAXO, "x", None, "Autre / Non classé", version="v9")
    ...
    try:
        op.urllib.request.urlopen = lambda req, timeout=None: _FakeResp({"message": {"content": content}})
        parsed = op.call_ollama_chat("http://x:11434", "m", "sys", "usr", 0.1, 8192, 5)
        check("client : parse JSON OK", isinstance(parsed, dict) and parsed.get("themes"), parsed)

        # JSON malformé dans content -> {} (repli géré par le mapping)
        op.urllib.request.urlopen = lambda req, timeout=None: _FakeResp({"message": {"content": "pas du json"}})
        bad = op.call_ollama_chat("http://x:11434", "m", "s", "u", 0.1, 8192, 5)
        check("client : content non-JSON -> {}", bad == {}, bad)

        # Service injoignable -> OllamaError (échec propre)
        def _boom(req, timeout=None):
            raise urllib.error.URLError("connection refused")
        op.urllib.request.urlopen = _boom
        raised = False
        try:
            op.call_ollama_chat("http://x:11434", "m", "s", "u", 0.1, 8192, 5)
        except op.OllamaError:
            raised = True
        check("client : Ollama down -> OllamaError", raised)
    finally:
        op.urllib.request.urlopen = orig_urlopen

    # ====================== O3 : registre & détection =======================
    from common.db import Base, SessionLocal, engine
    from common.models import MODEL_KIND_OLLAMA, ModelVersion
    from worker import model_registry as mr

    orig_list = op.list_ollama_models
    try:
        # 1. Moteur désactivé -> aucune entrée détectée.
        check("O3 : ollama désactivé -> None", mr._detect_ollama({"ollama": {"enabled": False}}) is None)

        cfg_oll = {"ollama": {"enabled": True, "model": "qwen2.5:7b", "base_url": "http://x:11434"}}

        # 2. Joignable + modèle présent -> available True.
        op.list_ollama_models = lambda *a, **k: ["qwen2.5:7b", "mistral:7b"]
        d = mr._detect_ollama(cfg_oll)
        check("O3 : modèle présent -> available", d and d["available"] is True, d)
        check("O3 : label = ollama:<model>", d["label"] == "ollama:qwen2.5:7b", d["label"])

        # 3. Joignable mais modèle absent -> available False + motif.
        op.list_ollama_models = lambda *a, **k: ["mistral:7b"]
        d2 = mr._detect_ollama(cfg_oll)
        ...
        def _down(*a, **k):
            raise op.OllamaError("refused")
        op.list_ollama_models = _down
        d3 = mr._detect_ollama(cfg_oll)
        check("O3 : Ollama injoignable -> indisponible", d3["available"] is False)
        check("O3 : metrics reachable=False", d3["metrics"]["reachable"] is False)

        # 5. Synchro complète en base : stub + entrée ollama (kind=ollama, available).
        Base.metadata.create_all(bind=engine)
        op.list_ollama_models = lambda *a, **k: ["qwen2.5:7b"]
        mr.sync_registry(cfg_oll)
        with SessionLocal() as db:
            oll_row = db.query(ModelVersion).filter_by(kind=MODEL_KIND_OLLAMA).one_or_none()
            check("O3 : entrée Ollama enregistrée", oll_row is not None and oll_row.available is True)
            check("O3 : stub reste actif par défaut (pas d'auto-activation Ollama)",
                  db.query(ModelVersion).filter_by(is_active=True).count() == 1
                  and oll_row.is_active is False)

        # 6. Re-synchro moteur désactivé -> l'entrée Ollama devient indisponible.
        mr.sync_registry({"ollama": {"enabled": False}})
        with SessionLocal() as db:
            oll_row = db.query(ModelVersion).filter_by(kind=MODEL_KIND_OLLAMA).one()
            check("O3 : désactivation -> Ollama neutralisé (available False)", oll_row.available is False)
    finally:
        op.list_ollama_models = orig_list

    # ====================== O4 : concurrence & robustesse ===================
    def make_predictor(max_parallel=4, retries=0):
        p = object.__new__(op.OllamaPredictor)
        p.cfg, p.taxonomy, p.sentiment_labels = CFG, TAXO, SENT_LABELS
        p.base_url, p.model = "http://x:11434", "m"
        p.temperature, p.num_ctx, p.timeout_s = 0.1, 8192, 5
    ...
    VALID = {"themes": [{"niv1": N1, "niv2": N2, "sentiment": "Neutre", "confidence": 0.8}],
             "signaux": {"rupture": False, "churn": False, "insatisfaction_forte": False}}
    orig_call = op.call_ollama_chat
    try:
        # 1. Pool : ordre préservé + verbatims vides court-circuités au bon index.
        op.call_ollama_chat = lambda *a, **k: dict(VALID)
        ...
        def _flaky(*a, **k):
            calls["n"] += 1
            if calls["n"] < 3:
                raise op.OllamaError("blip réseau")
            return dict(VALID)
        op.call_ollama_chat = _flaky
        ...
        # 3. Fail-fast : Ollama down -> OllamaError propagée (lot 'failed').
        op.call_ollama_chat = lambda *a, **k: (_ for _ in ()).throw(op.OllamaError("down"))
        raised = False
        try:
            make_predictor(max_parallel=4, retries=0).predict_cleaned_batch(["a", "b", "c", "d"], None)
        except op.OllamaError:
            raised = True
        check("O4 : Ollama down -> OllamaError propagée (échec propre)", raised)
    finally:
        op.call_ollama_chat = orig_call
